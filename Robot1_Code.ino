...
bool backToCornerState(int ts, int strictness)
{
  ...
  // Get info from wall sensor and update lSens and rSens
  lSens = analogRead(BACK_LEFT_SENSOR);
  rSens = analogRead(BACK_RIGHT_SENSOR);
...
bool isBallPresent() {
  ...
  white = colorSensor.getWhite();

  blue = (blue/((float)white)*100);
  if(blue <= 45 && blue >= 25){
    //Serial.println("ball is absent");
    return false;
  } else {
    return true;
  }
...
int getBallColor() {
  ...
  blue = (blue/((float)white)*100);
  if(blue > 45){
    //Serial.println("ball is white");
    return WHITE; // Check and make sure this is the right value
  } else if(blue < 25){
    //Serial.println("ball is orange");
    return ORANGE;
  }
...
void loop() {
  ...
  if(iterations == BLUETOOTH_LIMITER)
  {
    ...
    Serial3.print("Ball Present");
    Serial3.println(isBallPresent());
    Serial3.print(" Ball Color");
    Serial3.println(getBallColor());
    ...
    if(blue > 45){
    Serial3.println("ball is white");
  }
  if(blue <= 45 && blue >= 25){
    Serial3.println("ball is absent");
  }
  /* blue > 45 || blue < 25*/
  if(blue < 25){
    Serial3.println("ball is orange");
  }
